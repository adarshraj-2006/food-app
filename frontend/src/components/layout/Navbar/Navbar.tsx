...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white dark:bg-neutral-900 border-b border-neutral-100 dark:border-neutral-800 shadow-sm transition-all duration-300">
      ...
      {/* Menu Overlay */}
      {showMobileMenu && (
        <div className="fixed inset-0 top-[80px] bg-black/40 backdrop-blur-sm z-40 border-t border-neutral-200 dark:border-neutral-800 flex justify-end" onClick={(e) => {
          if (e.target === e.currentTarget) setShowMobileMenu(false);
        }}>
          <div className="w-full max-w-sm bg-white dark:bg-neutral-900 h-full overflow-y-auto p-6 space-y-8 animate-in slide-in-from-right duration-300 shadow-2xl border-l border-neutral-100 dark:border-neutral-800">
